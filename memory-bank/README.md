...
**Location**: `memory-bank/` (relative to workspace root `c:\Git\AgenticCoding\`)

> **Note**: The technical-writer agent mode defaults to `.memory-bank/` (dot prefix). This project uses `memory-bank/` (no dot prefix) because the folder also serves as a visible teaching artifact for the presentation.
