...
1. **Memory Bank** (`memory-bank/`) — Git-tracked, shared project context. Uses `memory-bank/` (no dot prefix) instead of the technical-writer default `.memory-bank/` because this folder also serves as a visible teaching artifact for the presentation. Core files: `projectbrief.md`, `productContext.md`, `activeContext.md`, `systemPatterns.md`, `techContext.md`, `progress.md`.
