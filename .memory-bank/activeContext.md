...
**Task**: Verified a failing `.\build.ps1 -ExportPptx -Version all -CheckOverflow` run and fixed the root cause. The build itself was healthy (30 / 73 / 138 slides); the PPTX export failed for all three versions with *"Failed to launch the browser process: Code: 0"* because Marp CLI bundles no browser, reads `CHROME_PATH`, and that variable was unset (Chrome absent; only Edge x86 installed). The overflow check worked because it uses the local `puppeteer` package's own downloaded Chromium. Fix in `content/pptx/Build-MarpVersions.ps1`: new `Resolve-MarpBrowserPath` helper + `CHROME_PATH` wiring in the export block (prefer explicit `CHROME_PATH` → `puppeteer.executablePath()` → installed Edge/Chrome; set for the export, restored after). Verified end-to-end: all three PPTX now export (1h 4.7 / 2h 12.9 / 4h 25.5 MB) and the overflow check completes for every version. The **exit code 1** the user saw is the overflow check acting as a CI gate over the **4 pre-existing 4h overflows** (Bottleneck / Deep Modules / Vigilance Trap / Stockfish) — not a build or export failure, and unrelated to last turn's root-level `build.ps1` wrapper. Two findings reported, not actioned: (a) the committed PPTX are **stale from Jun 11** (the deferred re-export is now unblocked); (b) the split-note merge is **non-deterministic** — a Module 9 appendix note block repositions between rebuilds (cosmetic .md churn). Regenerated decks/PPTX reverted to keep the commit focused. Branch `ai/agentic-gaps-research` (local only, not pushed). Repo milestone: **v1.1.0 released** (2026-06-07).
