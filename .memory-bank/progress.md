...
- 2026-08-13: **Fixed a presenter-voice bullet on *The Autonomy Horizon* (all three versions).** User reported the line "Teach it with its caveat — …" read like a side note rather than a bullet. Diagnosis was editorial, not CSS: the line was a true peer list item styled identically to its neighbours, but it addressed the **presenter** while the two bullets above addressed the **room**, so the voice switch read as a demotion in a slot reserved for a peer takeaway. Reworded to "**The caveat:** the self-reported *size* of the gain is debated; the *direction* is robust." and moved the presenter instruction into the slide's inline speaker notes, which now also name the "this is just AI hype" objection the caveat defuses — the claim itself is unchanged. Grep confirmed the defect is a **one-off** across the deck. Rebuilt **30 / 75 / 149** source slides, overflow **0 / 0 / 0**, note injection **115** (unchanged; no H1 touched, so the title map is intact). All three tracked PPTX re-exported (4.6 / 13.0 / 27.3 MB) — the 2h export first failed with `EBUSY` because the file was open in PowerPoint and succeeded after the user closed it; the process was not force-killed. Not committed per user.

- 2026-07-29: **CopilotAtelier upstream sync — Hooks as the fifth Customization type, Memory Bank routing, and a currency pass.** Scanned four upstream commits (2026-07-22 → 07-29) and implemented every finding with impact. **Two new slides.** (1) *Hooks — Enforcement That Doesn't Depend on Compliance* (**2h + 4h**, M3, after the ecosystem table) closes a genuine honesty gap: everything the curriculum taught about controlling behaviour — `copilot-instructions.md`, `.instructions.md`, `.agent.md`, `SKILL.md`, `.prompt.md` — is text the model *reads and may ignore*, whereas a hook is run by the host with a real exit-code contract (`0` allow / `2` block / other = non-blocking warn). (2) *Routing the Memory Bank — "Select", With a Number Attached* (**4h**, after *The Memory Bank Pattern*) turns the M2 *Select* verb into a measurable practice: `index.md` as the single unconditional read, an explicit fail-open rule, an authority order, and the fact that upstream gates its routing on **25 audited cases — zero critical misses and ≥ 50 % context reduction**. **Dual-use framing added** to *Containment Must Cover Host Trust Handoffs*: the `PreToolUse` hook that blocks your pushes is a script the Agent can edit, so it belongs outside auto-approved edit scope — the deck previously named hooks only as a threat. **Ecosystem table 6 → 7 types** (build source + split twin, `notes-title-map.psd1` alias updated in the same edit). **Template upgraded**: `content/materials/memory-bank-template/` gains `index.md` (routing map, fallback, authority order) and `decisions/0001-example-decision.md`; the README and `activeContext.md` corrected — `activeContext.md` is a status page, *not* "the index". **Handout gains a real incident**: *The guardrail that ate the guardrails* in `destructive-operations-guardrails.md` — upstream's setup script used `Remove-Item -Recurse` on a junction, which under Windows PowerShell 5.1 follows the reparse point and would have deleted the entire synced customization library; rare, silent, irreversible, self-targeting, and caught by an **independent security review of an unrelated change**, not by a test. **Currency**: Opus 4.8 → **Opus 5**, and — more useful than the version bump — agent examples now teach `model` as a **priority array** (first available wins, last entry GA) because hosted models retire on a ~6-week cadence; `agents: []` + `disable-model-invocation: true` taught explicitly (an unset `agents` key means "anyone may call me as a subagent"); the skill `description` cap corrected from a fictional "~500" to the real **hard 1024**; `agent-evals` reframed around the native Chat Customizations Evaluations extension + Microsoft **Waza** runner, with `run-evals.ps1` demoted to fallback; inventory refreshed to **~40 skills / five surfaces**; `plugin.json` distribution noted. Build: **30 / 75 / 149** source slides (from 30 / 74 / 147), overflow **0 / 0 / 0**, note injection **115**, build Pester **11/11**, PPTX re-exported at **31 / 76 / 150** slide parts with matching notes parts. Seven slides that gained content dropped one density tier; the already-compact *Skill-Authoring Discipline* slide was trimmed instead of shrunk. New slides PNG-verified. **Lesson recorded in `systemPatterns.md`**: renaming a split-file H1 silently breaks its `notes-title-map.psd1` alias and drops merged notes with no error (caught here as injection 115 → 114).

- 2026-07-29: **New 4h slide *Four Ways Context Goes Bad — Name It to Fix It* from a user-supplied context-engineering transcript.** Assessed the transcript: most of it was already covered and already better sourced — the existing M2 *Context Engineering* slide already carries context rot, just-in-time retrieval, notes-outside-the-window, and the literal phrase *curate, don't dump*; the seven-piece "context stack" is the repo's own Instructions / Memory Bank / MCP / output-format material rebadged; the `LogLook` example adds nothing over the existing demos. **One genuine gap**: the curriculum said context is finite but supplied **no vocabulary for how a long session actually degrades**. Implemented as a single 4h-only slide after the Context Engineering slide, naming **poisoning / distraction / confusion / clash** with observable symptoms. The transcript asserted these without evidence, so each row was re-verified against the two primary sources it derives from (Drew Breunig, *How Long Contexts Fail*, 2025-06-22; LangChain, *Context Engineering*, 2025-07-02) and the slide carries **measured effects instead of assertions**: Gemini 2.5 report (poisoned goals; degradation past ~100k tokens), Databricks (Llama 3.1 405b near 32k), GeoEngine (46 tools fail / 19 pass **inside the same 16k window** — attention, not capacity), Microsoft/Salesforce sharded prompts (**−39 %** average; o3 **98.1 → 64.1**). Two on-slide consequences: **every MCP server is a permanent context tax** (tool descriptions sit in the window on every call, used or not) — the evidence-backed counterweight to the "connect everything" framing in the MCP module — and the **Write / Select / Compress / Isolate** verbs, which unify four practices the curriculum already teaches in four separate places. Speaker notes state that the failure-mode→move pairing is a **teaching aid, not a canonical mapping**, and that compaction is lossy. Added a *Four context failures* row to the cheat-sheet *What's New to Teach* table. Not imported: the unsourced "hybrid retrieval beats embedding-only" claim and the implication that sub-agents are cheap. Rebuilt all three decks: 4h **146 → 147**, 1h/2h unchanged, overflow **0/0/0**, note injection 115, build Pester **11/11**; verified the slide appears only in `marp-4h-workshop.md`. CHANGELOG `[Unreleased] › Added` entry added. **Not committed** per user; tracked PPTX now stale (two new slides absent) — re-export offered, not run.
- 2026-07-29: **New 4h slide *MCP Sits On Top of Your APIs* from a user-supplied MCP-explainer transcript.** Assessed the transcript against existing coverage: most of it (USB analogy, N×M→N+M, self-describing schemas, least privilege, Linux Foundation) was already present or already stronger in the repo. Four framings were genuinely new and were consolidated into **one** 4h-only slide rather than four — MCP replaces the **middleware between the model and the API you already own**, not the backend; the **client is now the model**, so the caller is **probabilistic**; the contract is a tool *description*, not an endpoint; and **routing logic moves out of app code into the model's reasoning**, taking it out of unit-test coverage (hence evals + containment). Added on-slide honesty that auth/paging/rate-limit work **moved into the MCP server rather than vanishing** — the transcript's contrary claim is false and an operations audience will reject it. Also appended a rationale paragraph to the *MCP Security Considerations* split notes, recasting its safeguards as consequences of model-as-client (the tool description becomes attack surface; the call sequence is not something you specified). Deliberately **not** imported: WebSocket as an MCP transport (wrong — stdio + Streamable HTTP), the "MCP replaces APIs" hype framing, and the vague spec-keeps-you-safe claim. Rebuilt all three decks: 4h **145 → 146**, 1h/2h unchanged, overflow **0/0/0**, note injection 115, build Pester **11/11**; verified the slide and both note additions appear only in `marp-4h-workshop.md`. CHANGELOG `[Unreleased] › Added` entry added. **Not committed** per user; tracked PPTX now stale (new slide absent) — re-export offered, not run.
- 2026-07-22: **Implemented the July AOM web-gap refresh directly on `main`, intentionally uncommitted.** Added M3 *Three Evidence Planes* (2h/4h), plus 4h slides for Agent identity, host trust handoffs, and Memory Bank integrity; strengthened the Memory Bank template and destructive-operations handout; clarified the repository/workbench scope against newer enterprise AOM uses; added the MCP/A2A map; and refreshed VS Code 1.128/current Copilot model currency. Rebuilt all decks (**30 / 74 / 145** source slides), overflow **0 / 0 / 0**, Pester **11/11**, targeted PNG review clean, and PPTX re-exported with matching notes on all **31 / 75 / 146** rendered slides. No branch or commit created, per user instruction.
