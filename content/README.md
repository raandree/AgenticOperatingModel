...
- **instructions-template-repo/** — blueprint for the **team layer** of AI customization: repository layout for the five customization surfaces, the four distribution paths and what each one can and cannot carry (a `plugin.json` never ships Instructions or Hooks), a SemVer + version-marker update path, a **CI drift check** that makes the update path a required status check rather than a good intention, and a six-rule `GOVERNANCE.md` answering "who may change the rules?"
